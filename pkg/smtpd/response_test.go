...
func TestResponse(t *testing.T) {
	response := smtpd.Response{Code: smtpd.Code(250), Msg: []string{"OK"}}
	...
	response = smtpd.Response{Code: smtpd.Code(500), Msg: []string{"Not OK"}}
	...
	response = smtpd.Response{Code: smtpd.Code(500), Msg: []string{"Multi", "Lines"}}
	assert.Equal(t, "500-Multi\r\n500 Lines", response.String(), "Response is not parsed correctly")
	assert.Equal(t, true, response.IsError(), "Response code 500 indicates a failure")
	assert.Equal(t, false, response.IsSuccess(), "Response code 500 indicates a failure")
}
