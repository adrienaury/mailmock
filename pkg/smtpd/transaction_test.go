...
func TestTransactionNominal(t *testing.T) {
	...
	assert.NoError(t, err, "Initiated transactions MUST NOT return an error to a well-formed MAIL command")
	assert.NotNil(t, res, "Initiated transactions MUST return a response to a well-formed MAIL command")
	assert.Equal(t, int16(250), res.Code, "Initiated transactions MUST return response code 250 to a well-formed MAIL command")
	...
	assert.NoError(t, err, "In progress transactions MUST NOT return an error to a well-formed RCPT command")
	assert.NotNil(t, res, "In progress transactions MUST return a response to a well-formed RCPT command")
	assert.Equal(t, int16(250), res.Code, "In progress transactions MUST return response code 250 to a well-formed RCPT command")
	...
	assert.NoError(t, err, "In progress transactions MUST NOT return an error to a well-formed DATA command")
	assert.NotNil(t, res, "In progress transactions MUST return a response to a well-formed DATA command")
	assert.Equal(t, int16(354), res.Code, "In progress transactions MUST return response code 250 to a well-formed DATA command")
	...
	assert.Equal(t, int16(250), res.Code, "Data transactions MUST return response code 250 to a well-formed data transfer")
	...
	assert.Error(t, err, "Completed transactions MUST return an error after any valid command")
	assert.Nil(t, res, "Completed transactions MUST NOT respond to any valid command")
	assert.Equal(t, smtpd.TSCompleted, tr.State, "Completed transactions MUST NOT mutate state after any valid command")
	assert.Equal(t, []string{MailCommand.FullCmd, "250 OK", RcptCommand.FullCmd, "250 OK", DataCommand.FullCmd, "354 Start mail input; end with <CRLF>.<CRLF>", MailData[0], MailData[1], MailData[2], ".", "250 OK"}, tr.History, "Completed transactions MUST NOT update their history after any valid command")
	assert.Equal(t, smtpd.Mail{Envelope: smtpd.Envelope{Sender: "sender@example.com", Recipients: []string{"recipient@example.com"}}, Content: MailData}, tr.Mail, "Completed transactions MUST NOT update the mail after any valid command")
